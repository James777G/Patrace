...
import com.arm.pa.paretrace.Util.CPUFreqReader;
import com.arm.pa.paretrace.Util.CPUTempReader;
...
public class RetraceActivity extends Activity
{
    ...
    private final CPUFreqReader reader = new CPUFreqReader(this);

    private final CPUTempReader tempReader = new CPUTempReader(this);

    private final HashMap<Integer, View> viewIdToViewMap = new HashMap<Integer, View>();
    ...
    @Override protected void onCreate(Bundle icicle) {
        ...
        enOverlay = !force_single_window && parentIntent.getBooleanExtra("enOverlay", true);
        transparent = parentIntent.getIntExtra("transparent", 100);
        enFullScreen = parentIntent.getBooleanExtra("enFullScreen", false);


        if(SelectActivity.isTestMode) {
            reader.startRecording();
            tempReader.startRecording();
        }
    ...
    @Override protected void onDestroy() {
        ...
        if(SelectActivity.isTestMode) {
            ...
            tempReader.stopRecording();
            File externalCacheDir = getExternalCacheDir();

            if (CPUFreqReader.isFluctuated){
            ...
            if (externalCacheDir != null && externalCacheDir.isDirectory()) {
                ...
                if (files != null) {
                    for (File file : files) {
                        if(!"done.bin".equals(file.getName()) && !file.getName().endsWith(".gz")){
                            Log.d(TAG, "File name: " + file.getName());
                            CloudTraceResourceLoader.uploadFile(file, this);
                        }
                    }
                } else {
                    Log.d(TAG, "No files found or an I/O error occurred");
                }
            } else {
                Log.d(TAG, "External cache directory doesn't exist or is not a directory");
            }
